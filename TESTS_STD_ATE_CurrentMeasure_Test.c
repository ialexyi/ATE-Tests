...
TSTE_TEST ( STD_CURRENT_MEASURE_TEST )
{
	...
	LOG(CLB_LINE_HEADER_TEST, "Current Measure Test."); 
	...
	for( iIndex = 0; iIndex < iDB_PowerSupply_NumberOfChannel; iIndex++ )
	{
		...
		CHK_STDERR( DRV_PowerSupply_GetOutputVoltageActual ( ptPowerSupplyList[iIndex].hPowerSupplyHandle , ptPowerSupplyList[iIndex].iChannelNumber , &(ptPowerSupplyList[iIndex].lfMeasure_Voltage) )); 
		sprintf( szFormatedString , "Voltage Measure (%0.1lfV) : %0.3lf" , ptPowerSupplyList[iIndex].lfSetup_Voltage , ptPowerSupplyList[iIndex].lfMeasure_Voltage );
	...
	for( iIndex = 0; iIndex < iDB_PowerSupply_NumberOfChannel; iIndex++ )
	{
		...
		if ( bDB_EnableToStoreMeasureData )
		{
			sprintf( szFormatedVariable , "%s_TSeg_%d_TLoop_%d_Path_%d_Channel%d_AveragePower" , pCurrentTestPrefix , iCurrentTemperatureSegment , iCurrentTemperatureSegmentLoop , iCurrentPathIndex , (iIndex+1) );
			SetVarDouble( clb, VARTYPE_MEASURE_STORE , szFormatedVariable , (lfAverageValue * ptPowerSupplyList[iIndex].lfMeasure_Voltage)); 
		}
		
		RESULT( CLB_RESULT_DOUBLE , szFormatedString , (lfAverageValue * ptPowerSupplyList[iIndex].lfMeasure_Voltage) , "Average Power" ); 
		
		if ( bDB_EnableToStoreMeasureData )
		{
			sprintf( szFormatedVariable , "%s_TSeg_%d_TLoop_%d_Path_%d_Channel%d_MaximumPower" , pCurrentTestPrefix , iCurrentTemperatureSegment , iCurrentTemperatureSegmentLoop , iCurrentPathIndex , (iIndex+1) );
			SetVarDouble( clb, VARTYPE_MEASURE_STORE , szFormatedVariable , (ptPowerSupplyList[iIndex].lfMeasure_MaxValue * ptPowerSupplyList[iIndex].lfMeasure_Voltage) );
		}
		
		RESULT( CLB_RESULT_DOUBLE , szFormatedString , (ptPowerSupplyList[iIndex].lfMeasure_MaxValue * ptPowerSupplyList[iIndex].lfMeasure_Voltage) , "Maximum Pawer" );  
		
		if ( bDB_EnableToStoreMeasureData )
		{
			sprintf( szFormatedVariable , "%s_TSeg_%d_TLoop_%d_Path_%d_Channel%d_MinimumPower" , pCurrentTestPrefix , iCurrentTemperatureSegment , iCurrentTemperatureSegmentLoop , iCurrentPathIndex , (iIndex+1) );
			SetVarDouble( clb, VARTYPE_MEASURE_STORE , szFormatedVariable , (ptPowerSupplyList[iIndex].lfMeasure_MinValue * ptPowerSupplyList[iIndex].lfMeasure_Voltage) );
		}
		
		RESULT( CLB_RESULT_DOUBLE , szFormatedString , (ptPowerSupplyList[iIndex].lfMeasure_MinValue * ptPowerSupplyList[iIndex].lfMeasure_Voltage) , "Minimum Power" );  
	}
